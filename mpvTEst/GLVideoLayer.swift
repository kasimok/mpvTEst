...
func updateCallback(_ ctx: UnsafeMutableRawPointer?) {
    let videoLayer = unsafeBitCast(ctx, to: GLVideoLayer.self)
...
class GLVideoLayer: CAOpenGLLayer {
    ...
    func initMPV() {
        ...
        mpv_set_wakeup_callback(mpv, { (ctx) in
            let mpvController = unsafeBitCast(ctx, to: GLVideoLayer.self)
    ...
    let displayLinkCallback: CVDisplayLinkOutputCallback = { (displayLink, now, outputTime, flagsIn, flagsOut, displayLinkContext) -> CVReturn in
        let layer: GLVideoLayer = unsafeBitCast(displayLinkContext, to: GLVideoLayer.self)
